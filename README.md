...
- **Cloud CI/CD Timeouts & GDPR Walls:** When deployed on GitHub Actions, scrapers occasionally timed out due to slow virtual machines and massive GDPR cookie banners triggered by cloud IP addresses. This was resolved by extending the Playwright timeout to 60 seconds and configuring the QA block-detection logic to ignore GDPR banners so underlying article text could still be scraped.
